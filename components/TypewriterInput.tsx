    ...
    <div className="w-full max-w-4xl mx-auto px-2 pb-1 md:px-4 md:pb-4">
      {selectedImage && (
        <div className="mb-2 ml-1 relative inline-block">
          <img src={selectedImage} alt="Preview" className="h-10 w-10 md:h-16 md:w-16 object-cover rounded-xl border border-blue-500" />
          <button type="button" onClick={() => setSelectedImage(null)} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-0.5">
            <X size={10} />
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="flex items-end gap-1.5 md:gap-2">
        ...
        <button
          ...
          className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center rounded-xl md:rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-500 shrink-0"
        >
        ...
        <div className="relative flex-grow">
          <textarea
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && !e.shiftKey && (inputValue.trim() || selectedImage)) {
                e.preventDefault();
        ...
        {!isLoading ? (
          <button
            type="button"
            onClick={onMicClick}
            className={`w-10 h-10 md:w-14 md:h-14 flex items-center justify-center rounded-xl md:rounded-2xl shrink-0 ${
          ...
          <button
            type="button"
            onClick={onStop}
            className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center rounded-xl md:rounded-2xl bg-red-100 dark:bg-red-900/30 text-red-600 shrink-0"
          >
        ...
        <button
          ...
          className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center rounded-xl md:rounded-2xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 shrink-0 transition-transform active:scale-90"
        >
